...
const server = http.createServer((req, res) => {
    ...
    // Proxy Traffic (Check both blocklists for error page serving)
    if (proxyBlocklist.isBlocked(requestHost) || dnsBlocklist.isBlocked(requestHost)) {
        const category = 'Blocklist';
        const rule = proxyBlocklist.isBlocked(requestHost) ? 'Proxy blocklist' : 'DNS blocklist';
        const evidence = `Matched ${rule}`;
...
async function handleApiRoutes(req, res, pathName) {
    ...
    if (pathName === '/api/users' && req.method === 'POST') {
        ...
        if (!username) {
            res.writeHead(400, { 'Content-Type': 'application/json' });
            res.end(JSON.stringify({ success: false, error: 'Invalid username format (3-50 alphanumeric characters)' }));
            return true;
        }
